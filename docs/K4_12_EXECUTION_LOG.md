...
| NET-01 | IN_PROGRESS | afc4e3b2, 2ab9d22f | GitHub Actions run `30717667553` PASS: Kobo host, CTest, dependency-audit, cppcheck en clang-format; local macOS host build remains blocked by missing Linux headers | N437 timing evidence ontbreekt | Saved-network service is rate-limited to 500 ms and DHCP is now launched as a managed child process instead of synchronously from `WiFi.status()`; asynchronous scan and measured latency remain open. |

...
| NET-01 | IN_PROGRESS | afc4e3b2, 2ab9d22f, 3ecf5d52 | GitHub Actions run `30717826380` PASS: Kobo host, CTest, dependency-audit, cppcheck en clang-format; local macOS host build remains blocked by missing Linux headers | N437 timing evidence ontbreekt | Saved-network service is rate-limited to 500 ms; DHCP and `iw` scan now run as managed child processes polled without blocking `WiFi.status()`/`scanComplete()`; measured latency remains open. |
...
commit=2ab9d22f
remaining_risk=The child-process lifecycle is host-CI validated, but scan/DHCP latency, cancellation under driver failure and reconnect behavior must still be measured on the N437 before NET-01 can be FIXED_LOCAL.
```

### Fase 4 vervolg — asynchronous Wi-Fi scan

```text
status=IN_PROGRESS
root_cause=scanComplete() executed iw dev wlan0 scan synchronously despite the async API surface, blocking the activity/main loop during driver scan time.
implementation=3ecf5d52 starts iw with fork/exec, redirects output to a private 0600 staging file, polls completion with waitpid(WNOHANG), parses only after completion, and bounds cancellation on scanDelete/disconnect.
tests=GitHub Actions run 30717826380: Kobo host, CTest, dependency-audit, cppcheck en clang-format PASS.

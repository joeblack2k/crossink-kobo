...
| Wi-Fi | Scan/join, bekende netwerk-autoconnect, statusicoon | PASS | test-netwerk-autoconnect en live status bewezen |
| Wi-Fi | Altijd-aan lifecycle tijdens alle netwerkactivities | PARTIAL | N437 bewees een expliciete Wi‑Fi-scan met behoud van het test-netwerk, SSH, `wpa_supplicant` en HTTP-upload vóór én na Back. Clock, fonts, OPDS-browser, Calibre en KOReader-auth zijn Kobo-no-teardown. KOReader-sync, webactivity en nearby blijven open. [bewijs](../artifacts/kobo/hardware/network/20260712T0843Z/manifest.txt) |

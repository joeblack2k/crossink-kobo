...
| 20 | Kobo Device Info | `Systeem → Apparaat → Apparaat` openen via touch; Wi-Fi-status zichtbaar; Back via touch | `artifacts/kobo/hardware/device-info/20260711T1930Z/` | PASS op `16768646944e`; SSID `Legacy`, IPv4, RSSI, model, kernel, opslag, batterij en frontlight zichtbaar |
...
| 25 | Netwerklifecycle | Voer clock, fonts, OPDS, Calibre, KOReader en sync uit terwijl bekende Wi-Fi en webserver actief blijven | `artifacts/kobo/hardware/network/20260712T0843Z/manifest.txt` + netwerk-failurematrix | PARTIAL op `53f2cfd7d48d` — N437 Wi‑Fi-scan, SSH, `wpa_supplicant` en HTTP-upload bleven tijdens én na picker-Back op `Legacy` beschikbaar; overige flows nog niet E2E |
